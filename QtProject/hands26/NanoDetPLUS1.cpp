...
bool NanoDetPLUS1::hasGPU = true;
NanoDetPLUS1* NanoDetPLUS1::detector = nullptr;
...
NanoDetPLUS1::NanoDetPLUS1(const char* param, const char* bin, bool useGPU)
{
...
NanoDetPLUS1::~NanoDetPLUS1()
{
    delete this->Net;
}

std::vector<BoxInfo> NanoDetPLUS1::detect(cv::Mat& image, float score_threshold, float nms_threshold)
{
...
void NanoDetPLUS1::draw(cv::Mat& image, std::vector<BoxInfo> boxes)
{
...
void NanoDetPLUS1::preprocess(cv::Mat& image, ncnn::Mat& in)
{
...
void NanoDetPLUS1::decode_infer(
    ncnn::Mat& feats, 
    std::vector<CenterPrior>& center_priors, 
    float threshold, 
    std::vector<std::vector<BoxInfo>>& results, 
...
BoxInfo NanoDetPLUS1::disPred2Bbox(const float*& dfl_det, int label, float score, int x, int y, int stride, float width_ratio, float height_ratio)
{
    float ct_x = x * stride;
    float ct_y = y * stride;
...
BoxInfo NanoDetPLUS1::disPred2Bbox(const float*& dfl_det, int label, float score, int x, int y, int stride)
{
...
void NanoDetPLUS1::nms(std::vector<BoxInfo>& input_boxes, float NMS_THRESH)
{
